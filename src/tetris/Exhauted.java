 ...
 * @author SON
 */
public class Exhauted extends Scene {

    private static Exhauted current;
...
//        backGround.setVisible(false);
    ...
    public static Exhauted createScene(Stage primaryStage) {
        board = new Board(10, 22, Name.name);
//        board.disableVirtualPiece();
        Exhauted.root = new Group();
        Exhauted result = new Exhauted(Exhauted.root);
        Exhauted.root.getChildren().add(createBackGound("bg1.jpg", result));
        ...
        Exhauted.root.getChildren().addAll(xxx, attention);
        Exhauted.primaryStage = primaryStage;
        return result;
    }
                ...
                if (timer.getStatus() == Timeline.Status.RUNNING) {
                    ...
                    if (event.getCode() == KeyCode.DOWN || event.getCode() == KeyCode.NUMPAD0 || event.getCode() == KeyCode.SPACE) {
                        try {
                            ...
                            Logger.getLogger(Exhauted.class.getName()).log(Level.SEVERE, null, ex);
                        }
                    }
                }
            } else {
